...
import { decodeManifest, parseManifestV12 } from "../src/lib/manifest-codec";
import { desktopSnapshot } from "./fixtures";

function manifestV12() {
...
describe("OPFS manifest codec", () => {
  test("round-trips the persisted v12 shape", () => {
    const input = manifestV12();
    expect(parseManifestV12(JSON.parse(JSON.stringify(input)))).toEqual(input);
  ...
  test("migrates v11 without ordering metadata and preserves signed coordinates", () => {
    ...
    expect(decoded.manifest.version).toBe(12);
  ...
  test("rejects invalid v12 sync and non-finite positions", () => {
    expect(() => parseManifestV12({ ...manifestV12(), sync: { ...manifestV12().sync, revision: -1 } })).toThrow("revision");
    expect(() => parseManifestV12({ ...manifestV12(), entries: [{ kind: "folder", id: "a", name: "A", parentId: null, modifiedAt: 1, position: { x: Number.NaN, y: 0 } }] })).toThrow("position");
  });
});
