...
Authoritative file bytes are opaque blobs under `.hiraya-data/files/<entry-id>`. User-visible names, folders, parent relationships, stable IDs, and revisions live in the WAL-enabled `.hiraya-data/workspace.sqlite` database; folders do not create physical directories. The HTTP workspace schema is version 5. Existing `workspace.json` metadata is validated and imported automatically on first startup. A restart-safe one-time migration converts the previous logical-path file tree to ID blobs, copying only files referenced by workspace metadata and rejecting missing, size-mismatched, symbolic-link, or non-regular content. The previous tree is retained as a hidden `.legacy-logical-files-recovery-*` directory for manual recovery and can be removed after the migrated workspace is verified.

The `files` directory is private server storage, not a user-editable file tree. Out-of-band filesystem changes are not imported or synchronized; use Hiraya's HTTP API or UI for all workspace changes.
