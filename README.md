...
Desktop workspaces are responsive pages rather than persisted containers. Saved icon coordinates form a continuous logical surface: each browser preserves them exactly and projects overflow into later virtual workspaces at the corresponding local position. Enlarging the viewport restores the original arrangement without persisting a resize-specific layout. Capacity can add pages, and dragging an icon through any outer desktop edge creates a non-empty adaptive workspace. Edge-created workspaces remain separate at the creation capacity and on smaller screens, then merge on roomier devices. Empty workspaces are never stored. Reordering workspace pages updates the shared root icon order, then every device repaginates that order for its own resolution.
