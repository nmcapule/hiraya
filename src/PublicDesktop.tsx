...
import { DownloadSimple, File as FileGlyph, Folder, SignIn, SpinnerGap, WarningCircle, X } from "@phosphor-icons/react";
...
export default function PublicDesktop({ token }: { token: string }) {
  ...
  return <main className="desktop-shell public-desktop" data-theme={isBuiltinThemeId(appearance.selectedThemeId) ? appearance.selectedThemeId : "custom"} style={themeStyle(theme)}>
    <header className="menu-bar public-menu"><div className="brand-mark"><span className="brand-mark__shape"><span /></span><strong>Hiraya</strong><span className="public-menu__desktop">{desktop?.name || "Public desktop"}</span></div><div className="public-menu__actions"><span className="readonly-badge">Read only</span><a className="button button--quiet" href={loginUrl()}><SignIn size={16} /> Sign in</a></div></header>
    <section className="desktop public-desktop__surface" data-wallpaper={desktop?.layout.wallpaper.source.startsWith("file:") ? "dusk" : desktop?.layout.wallpaper.source ?? "dusk"} aria-label={desktop ? `${desktop.name} public desktop` : "Public desktop"}>
      <div className="wallpaper-image" aria-hidden="true" /><div className="wallpaper-grain" aria-hidden="true" />
      ...
      {desktop && <div className="public-icon-grid">{roots.map((entry) => <button className="public-icon" type="button" key={entry.id} aria-pressed={selectedIds.has(entry.id)} onClick={() => setSelectedIds(new Set([entry.id]))} onDoubleClick={() => openEntry(entry)} onKeyDown={(event) => { if (event.key === "Enter") openEntry(entry); }}>{entry.kind === "folder" ? <Folder size={39} weight="duotone" /> : <FileGlyph size={39} weight="duotone" />}<span>{entry.name}</span></button>)}</div>}
      {open && <div className="app-window-layer"><AppWindow id="public-view" title={open.kind === "folder" ? folder?.name ?? desktop?.name ?? "Desktop" : open.file.name} titleId="public-view-title" bounds={{ x: 90, y: 55, width: Math.min(920, window.innerWidth - 40), height: Math.min(680, window.innerHeight - 90) }} zIndex={1} focused minimized={false} segmentActive mobile={mobile} onFocus={() => undefined} onBoundsChange={() => undefined} onMinimize={() => undefined} onClose={() => setOpen(null)} titleArea={<div><span className="window-kicker">Public · Read only</span><h2 id="public-view-title">{open.kind === "folder" ? folder?.name ?? desktop?.name : open.file.name}</h2></div>}>
        {(headerElements) => open.kind === "folder" ? <FolderExplorer folder={folder ?? null} rootLabel={desktop?.name ?? "Desktop"} breadcrumbs={folder ? index.ancestors(folder.id).filter((entry): entry is FolderEntry => entry.kind === "folder") : []} children={index.children.get(open.folderId) ?? []} selectedIds={selectedIds} onSelect={(entry) => setSelectedIds(new Set([entry.id]))} onNavigate={(next) => setOpen({ kind: "folder", folderId: next?.id ?? null })} onOpen={openEntry} onCreateFolder={() => undefined} onCreateFile={() => undefined} onUpload={() => undefined} onMove={() => undefined} onContextMenu={() => undefined} onBlankContextMenu={() => undefined} readOnly headerElements={headerElements} /> : open.blob ? <FileWindow file={open.file} blob={open.blob} editable={fileCapabilities(open.file).editable} readOnly headerActionsTarget={headerElements.actions} editorSettings={desktop?.editorSettings ?? DEFAULT_EDITOR_SETTINGS} externalEmbeddedPreviews={false} theme={theme} onSave={async () => undefined} onDownload={() => void loadFile(open.file, true)} onEdit={() => undefined} onEditorSettingsChange={() => undefined} onResolveLink={(path) => resolveLinkedFile(open.file, path)} onOpenLinkedFile={(file) => void loadFile(file)} /> : open.error ? <div className="app-window__loading" role="alert"><span>{open.error}</span><button className="button button--primary" type="button" onClick={() => void loadFile(open.file)}>Retry</button></div> : fileCapabilities(open.file).preview === "none" ? <div className="no-preview"><p>No preview is available for this file type.</p><button className="button button--primary" type="button" onClick={() => void loadFile(open.file, true)}><DownloadSimple size={16} /> Download file</button></div> : <div className="app-window__loading" role="status"><SpinnerGap size={22} /> Opening {open.file.name}...</div>}
      </AppWindow></div>}
    </section>
    {gate && <div className="sharing-dialog__backdrop"><section className="large-download-gate" role="dialog" aria-modal="true" aria-labelledby="download-gate-title"><button className="icon-button" type="button" onClick={() => setGate(null)} aria-label="Close"><X size={18} /></button><span className="large-download-gate__icon"><DownloadSimple size={25} /></span><h2 id="download-gate-title">Sign in for this download</h2><p><strong>{gate.fileName}</strong> is large enough to require an authenticated download. The public desktop remains available.</p><a className="button button--primary" href={new URL(gate.loginUrl, window.location.href).href}><SignIn size={16} /> Sign in and return</a></section></div>}
  </main>;
}
